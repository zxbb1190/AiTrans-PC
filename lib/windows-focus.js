...
$wasMinimized = [AiTransFocus]::IsIconic($hwnd);
if ($wasMinimized) {
...
[Console]::Out.WriteLine((@{
    ok = $restored;
    reason = $(if ($restored) {
        if ($wasMinimized) { "restored_minimized_window" } else { "restored_foreground_only" }
    } else { "set_foreground_failed" });
...
`);
  ...
  return {
    ...
    wasMinimized: Boolean(payload.wasMinimized),
  };
}
